# =============================================================================
# Guild-specific ATT- Nigeria

# : PSM + WLS
# : PSM

# Nigeria: bio1_trend, bio12_trend


#   - guild richness(S0, S3, change) × 3 guilds
#   - Nigeriashapefile
#   - PSM 1:1 matched data
#   - 


#   - Nigeria_guild_richness_by_LGA.csv
#   - Nigeria_guild_DR_ATT_results.csv (ATT)
#   - Nigeria_matched_with_guild_richness.csv
# =============================================================================
...
cat("\n")
cat("╔══════════════════════════════════════════════════════════════════════╗\n")
cat("║     Nigeria Guild-specific ATT                           ║\n")
cat("║     : PSM + WLS (Δbio1, Δbio12)                            ║\n")
cat("╚══════════════════════════════════════════════════════════════════════╝\n\n")
...
guild_raster_dir <- "D:/OneDrive - University of Leeds/biodiversity_manscript/results/Nigeria/guild_richness"
admin_shp <- "D:/OneDrive - University of Leeds/biodiversity_manscript/dataset/shp/Nigeria_adm2.shp"
psm_file <- "E:/2026.1.8_biomod2/nigeria_results/PSM_1to1/Nigeria_1to1_matched_data.csv"
climate_file <- "E:/2026.1.8_biomod2/nigeria_results/PSM_1to1_with_trend/matched_data.csv"


output_dir <- "E:/2026.1.8_biomod2/nigeria_results/guild_analysis"
...
guild_by_lga <- data.frame(GID_2 = admin[[id_col]])
...
for(raster_name in names(guild_rasters)) {
  ...
  guild_by_lga[[raster_name]] <- extracted[, 2]
}
...
for(guild in guilds) {
  ...
  if(s3_col %in% names(guild_by_lga) && s0_col %in% names(guild_by_lga)) {
    guild_by_lga[[change_col]] <- guild_by_lga[[s3_col]] - guild_by_lga[[s0_col]]
  }
}
...
guild_lga_file <- file.path(output_dir, "Nigeria_guild_richness_by_LGA.csv")
write.csv(guild_by_lga, guild_lga_file, row.names = FALSE)
cat(sprintf("  ✓ : %s\n", guild_lga_file))
...
matched_guild <- matched_data %>%
  left_join(guild_by_lga, by = "GID_2")
...
cat("【Guild-specific ATT (Δbio1, Δbio12)】\n")
cat("─────────────────────────────────────────────────────────────────────────\n")
...
att_file <- file.path(output_dir, "Nigeria_guild_DR_ATT_results.csv")
write.csv(att_results, att_file, row.names = FALSE)
cat(sprintf("  ✓ %s\n", att_file))


balance_climate$Country <- COUNTRY
balance_file <- file.path(output_dir, "Nigeria_guild_climate_balance.csv")
write.csv(balance_climate, balance_file, row.names = FALSE)
cat(sprintf("  ✓ %s\n", balance_file))


matched_guild_file <- file.path(output_dir, "Nigeria_matched_with_guild_richness.csv")
